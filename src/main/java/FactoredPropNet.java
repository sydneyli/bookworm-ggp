...
public final class FactoredPropNet
{
	...
	public Set<Proposition> relevantPropositions;

	private void initFromComponents() {
	 ...
	 */
	public FactoredPropNet(List<Role> roles, Set<Component> components)
	{
	    this.roles = roles;
		this.components = components;
		initFromComponents();
		this.renderToFile("best_lights_factored.dot");
		System.out.println("[PropNet] Factoring...");
		this.relevantPropositions = factorPropnet();
		initFromComponents();

// 		System.out.println("[PropNet] Base: " + basePropositions.size());
// 		for (Proposition prop : irrelevant) {
// 			basePropositions.remove(prop.getName());
// 			inputPropositions.remove(prop.getName());
// 		}
//
// 		Set<Proposition> relevantLegalProps = inputPropositions.values().stream().map(legalInputMap::get).collect(Collectors.toSet());
//
// 		for (Role r : roles) {
// 			Proposition pseudoNoop = legalPropositions.get(r).stream().findAny().get();
// 			legalPropositions.get(r).retainAll(relevantLegalProps);
// 			if (legalPropositions.get(r).isEmpty()) {
// 				legalPropositions.get(r).add(pseudoNoop);
// 			}
// 		}
		System.out.println("[PropNet] Finished factoring!");
	}

	private Optional<Proposition> toLegal(Component c) {
		if (c instanceof Proposition) {
			Proposition p = (Proposition) c;
			if (inputPropositions.containsKey(p.getName())) {
				return Optional.of(legalInputMap.get((Proposition)c));
			}
		}
		return Optional.empty();
	}

	private void expandInputs(Set<Component> visited) {
		Queue<Component> explore = new LinkedList<>(visited);
		Set<Component> newVisited = new HashSet<>();
		while (!explore.isEmpty()) {
			Component neighbor = explore.remove();
			if (newVisited.contains(neighbor)) continue;
			newVisited.add(neighbor);
			visited.add(neighbor);
			explore.addAll(neighbor.getInputs());
		}
	}

	private Set<Proposition> factorPropnet() {
		Queue<Component> explore = new LinkedList<>();
		explore.add(terminalProposition);
		// explore.add(initProposition);
		Set<Component> visited = new HashSet<>();
		while (!explore.isEmpty()) {
			Component neighbor = explore.remove();
			if (visited.contains(neighbor)) continue;
			toLegal(neighbor).ifPresent(result -> visited.add(result));
			visited.add(neighbor);
			explore.addAll(neighbor.getInputs());
			explore.addAll(neighbor.getOutputs());
		}
		expandInputs(visited);

		Set<Component> irrelevant = new HashSet<>(components);
		irrelevant.removeAll(visited);
		for (Component c : irrelevant) {
			this.removeComponent(c);
		}
		return visited.stream().filter(x -> x instanceof Proposition).map(x -> (Proposition)x).collect(Collectors.toSet());
	}
